(function() {
	...
	window.App = new Vue({
		...
		el: '#App',
		data: {
            upline: 'TFtWLBs6bVXUXbEtgmn5QLW5PU1LZrPaqJ', // TKxumm6pmZiXSqRkr4ggrwdGwbPi6r2VW2 - Zero address
            contract_address: 'TSLaYWjzXTNaQcPE1j6tagF6vqtStNtz1x', // TTzv1Vpnvpqu6wCqzUFZpSVnBrunEiSeGA
            contract: {
                invested: 0,
                withdraw: 0,
                direct_bonus: 0,
		...
		mounted() {
		    ...
		    if(!document.cookie.match(/coopolice=1/)) {
		    	this.notice('We use cookies to do things like offer live chat support and show you content. If you agree to our use of cookies, please continue to use our site.🍪', '004e92',  0).then(() => (document.cookie = 'coopolice=1; Max-Age=31536000; path=/'));
		    }
		...
		methods: {
           ...
           notice(msg, color = '004e92', time = 3000) {
            return new Promise((resolve, reject) => {
                let wrap = $('<div style="position:fixed; left:calc(50% - 150px); box-shadow:0 5px 25px rgba(0,0,0,0.2); width:320px; top:40px; background:#' + (color ? color : '004e92') + '; border-radius:10px; color:#fff; padding:20px 20px; font:14px/1.2 Tahoma, sans-serif; cursor:pointer; z-index:999999; text-align:center;">' + msg + '</div>')
        ...
        copyText(value) {
            ...
            try {
                ...
                this.notice('Link copied to clipboard', '4ca1af');
            } catch (err) {}
